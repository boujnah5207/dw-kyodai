...
kyodai.linex = function(x1, x2, y)
{
  ...
  {
    while (x1++ < x2)
    path.push('<img src="images/linex.gif" style="position:absolute;left:'+(x1*35-18)+'px;top:'+y*35+'px"/>')
  }
  else
  {
    while (x2++ < x1)
    path.push('<img src="images/linex.gif" style="position:absolute;left:'+(x2*35-18)+'px;top:'+y*35+'px"/>')
  }
  return path
}
...
kyodai.liney = function(y1, y2, x)
{
  ...
  {
    while (y1++ < y2)
    path.push('<img src="images/liney.gif" style="position:absolute;left:'+x*35+'px;top:'+(y1*35-18)+'px"/>')
  }
  else
  {
    while (y2++ < y1)
    path.push('<img src="images/liney.gif" style="position:absolute;left:'+x*35+'px;top:'+(y2*35-18)+'px"/>')
  }
  return path
}
...
kyodai.setting = function(arr)
{
  ...
  for (i=0; i<kyodai.shape.length; i++)
  {
    ...
    if (Img)
    {
      itemImg.push('<img id=Item_'+x+'_'+y+' src="images/wow/'+ Img + '.png" style="z-index:'+ (100-x+y) +';position:absolute;left:'+ x*35 +'px;top:'+ y*35 +'px">')
    }
  }
    ...
    if (n)
    {
      for (var j=i+1; j<s.length; j++)
      {
        if (n == kyodai.block[s[j].x+","+s[j].y])
        {
          ...
          if (line)
          {
            $("#kyodai_cuechoose").html('<img src = "images/choose.gif" onmouseup="kyodai.click('+sx+','+sy+')" style="position:absolute;left:'+ (sx*35) +'px;top:'+ sy*35 +'px">'
            + '<img src = "images/choose.gif" onmouseup="kyodai.click('+ex+','+ey+')" style="position:absolute;left:'+ (ex*35) +'px;top:'+ ey*35 +'px">');
            $("#kyodai_lines").html(line.join(""));
            if (isbomb)
            {
